package com.hinish.spec.iab.vast.parsers
{
    ...
    public class VASTParser
    {
        ...
        private function parseLinear(node:XML):Linear
        {
			...
			if(node.Duration as String)
            l.duration = new Time(node.Duration);
			//var kkkkkk:String=node.@skipoffset;
			if(String(node.@skipoffset)){
				var offset_value:String=node.@skipoffset;
				if(offset_value.indexOf(":")!=-1){
					var ar:Array=offset_value.split(":");
					if(ar.length==3) 
						l.skipoffset = int(ar[0])*60*60+int(ar[1])*60+int(ar[2]);
					if(ar.length==2) 
						l.skipoffset = int(ar[0])*60+int(ar[1]);
					if(ar.length==1) 
						l.skipoffset = int(ar[0]);
					
				}
				if(offset_value.indexOf("%")!=-1){
					l.skipoffset=Number(offset_value.replace("%",""))/100;
				}
        ...
        private function parseNonLinear(node:XML):NonLinear
        {
			...
			if(node.@minSuggestedDuration){
				var min_duration:Number=0;
				var duration_string:String=node.@minSuggestedDuration;
				if(duration_string.indexOf(":")!=-1){
					var ar:Array=duration_string.split(":");
					if(ar.length==3){
						min_duration=int(ar[0])*60*60+int(ar[1])*60+Number(ar[2]);
					}
					if(ar.length==2){
						min_duration=int(ar[0])*60+Number(ar[1]);
					}
				}else min_duration=Number(min_duration);
				nl.minSuggestedDuration=min_duration;
			}else{
