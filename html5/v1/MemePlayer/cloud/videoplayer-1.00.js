                ...
                ping: function (vid, session, data) {
                    ...
                    if (e && e.contentWindow) {
						if (window.top != window){
							var ar = data.params.split('&');
							for (var i = 0; i< ar.length; i++){
								var p = ar[i].split('=');
								if (p[0] == 'ref'){
									ar[i] = 'ref='+document.referrer;
									break;
								}	
							}
							data.params = ar.join('&');
                        ...
                        if (!tracked[session] && obj.ev === 'p') {
                            ...
                            setTimeout(function () {
                                ...
                                for (var ii = 0; ii < gaArr.length; ii++){
                                    ga('create', gaArr[ii], 'auto');
                                }
                                ga('send', 'pageview');
                            }, 100);
                        }
    ...
    if (ua.pv[0] >= 10) { // use Flash
        w.MeCloudVideoPlayer = function (ele, data) {
            ...
            if (ele.offsetWidth > 400)
                embed.style.setProperty('min-height', "300px", "important");
