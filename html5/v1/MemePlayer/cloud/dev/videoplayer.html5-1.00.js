(function(w, d) {
    ...
    var MemeVideoAds = function(stageComponent, adComponent, player) {
        ...
        function onAdEvent(adEvent) {
            ...
            switch (adEvent.type) {
                case google.ima.AdEvent.Type.LOADED:
                    console.log("[MeCloudPlayer]", "Ad loaded ", ad);
                ...
                case google.ima.AdEvent.Type.STARTED:
                    ...
                    if (ad.isLinear() || !player.isPlaying) {
                        ...
                        if (!ad.b.vpaid) {
                            ...
                            isVpaidPlaying = true;
                            player.components.adSkip.style.setProperty("top", (isMobile ? isIphone ? size.height : size.height + 40 : size.height + 15) + "px", "important");
                        }

                        if (player.components.subtitleBtn.checkSubOn() === true) {
                            player.components.subtitleBtn.hideSubtitle();
                        }
                        var state = d.fullScreen || d.mozFullScreen || d.webkitIsFullScreen || d.msFullscreenElement;
                        if (state)
                            player.components.ad.element.style.setProperty("height", "100%", "important");
                    }
                    ...
                    if (adHeight < 91 && adHeight > 19) {
                        if (adsInfo.position !== undefined) {
                            ...
                            player.components.subtitleBtn.setSubLine();
                        }
                        player.components.ad.aw = ad.b.naturalWidth;
                ...
                case google.ima.AdEvent.Type.USER_CLOSE:
                    ...
                    player.hideAdControls();
                    player.components.ad.clearAdContainer();
                    if (adsInfo.position !== (undefined && MIDROLL)) {
                        onAdEnd();
                    } else {
                        if (ad.isLinear()) {
                            playVideo();
        ...
        function onAdEnd() {
            console.log('AD END');
            if (player.components.subtitleBtn) {
                player.components.subtitleBtn.setDefaultSubLine();
            }
            adsManager && adsManager.destroy();
            adDisplayContainer = null;
            isVpaidPlaying = false;
        ...
        function onContentResumeRequested() {
            ...
            if (player.components.subtitleBtn.checkSubOn() === true) {
                player.components.subtitleBtn.showSubtitle();
            }
    ...
    MemeVideoComp.create("stage", {
        ...
        setTracks: function(tracks) {
            ...
            for (i = 0; i < tracks.length; i++) {
                ...
                t.src = tracks[i].file;
                t.label = tracks[i].label;
                this.subLabels.push(t.label);
                this.subFiles.push(t.src);
    ...
    MemeVideoComp.create("timeDisplay", {
        ...
        onResize: function() {
            ...
            //  console.log(this.player.size.width + ' ' + this.player.size.height);
        }
    });
    ...
    MemeVideoComp.create("subtitleBtn", {
        ...
        parseSubtitleSrt: function(tracks) {
            ...
            console.log(this.subtitleSrtArray);
        },
        ...
        init: function() {
            console.log("CREATE SUB BTN");
            var e = this,
                t = this.element = d.createElement("div"),
                i = this.player,
                isMobile = i.displayMobileMode();
            e.videoTag = this.player.components.box.element.childNodes[1];
            ...
            e.subConfigFrame.onclick = function(ev, i) {
                ev.stopPropagation();
                var id = ev.path[1].id;
                if (id === 'btn-option-language') {
                ...
                } else if (id === 'btn-option-subtitles') {
                    console.log('option sub frame');
                    ...
                    track = e.videoTag.textTracks[e.currentLang];
                    if (checkOnOff.checked == true) {
                        ...
                        document.getElementById('btn-option-language').childNodes[1].innerHTML = 'OFF ›';
                        ...
                        document.getElementById('btn-option-language').childNodes[1].innerHTML = e.languagesArray[e.currentLang] + ' ›';
            ...
            f.innerHTML = '<ul id="default-sub-frame"><li><div class="title-config" style="padding: 5px 10px !important">Subtitles</div><div class="active-config" style="padding: 5px 10px !important"><div class="switch-checkbox"><input id="sc-toggle-1" class="sc-toggle sc-toggle-round-flat" type="checkbox"><label for="sc-toggle-1"></label></div></div></li><li id="btn-option-language"><div class="title-config" style="padding: 5px 10px !important">Languages (0)</div><div class="active-config current" style="padding: 5px 10px !important">OFF ›</div></li><li id="btn-option-subtitles"><div class="title-config" style="padding: 5px 10px !important">Option</div><div class="active-config" style="padding: 5px 10px !important">›</div></li></ul>';
            ...
            e.languagesFrame.onclick = function(ev) {
                ...
                if (id === "btn-back-subtitles") {
                    ...
                    var dd = document.getElementById('btn-option-language'),
                        ...
                        btnSub = document.getElementById('btn-subtitles'),
                        len = e.videoTag.textTracks.length;
                    for (i = 0; i < len; i++) {
                        e.videoTag.textTracks[i].mode = 'hidden';
                    }
                    dd.childNodes[1].innerHTML = lang + ' ›';
                    if (lang === 'OFF') {
                        checkOnOff.checked = false;
                        btnSub.classList.remove('active');
                        e.videoTag.textTracks[e.currentLang].mode = 'hidden';
                        e.setSubOff();
                        e.hideSrtSub();
                    } else {
                        checkOnOff.checked = true;
                        btnSub.classList.add('active');
                        e.videoTag.textTracks[e.currentLang].mode = 'showing';
                        e.setSubOn();
                        e.showSrtSub();
                    }
            ...
            e.optionsFrame.onclick = function(ev) {
                ...
                if (id === 'btn-sub-options-reset') {
                    ...
                    sheet = style.sheet;
                    sheet.addRule('.memeplayer-video::cue', 'font-family: ' + fontFamily);
                    sheet.addRule('.memeplayer-video::cue', 'font-size: ' + fontSize + 'px');
                    sheet.addRule('.memeplayer-video::cue', 'color: rgba(255, 255, 255, 1)');
                    sheet.addRule('.memeplayer-video::cue', 'background-color: rgba(255, 255, 255, 0)');
                    sheet.addRule('.memeplayer-video::cue', 'text-shadow: 2px 0 0 #000000, -2px 0 0 #000000, 0 2px 0 #000000, 0 -2px 0 #000000, 1px 1px #000000, -1px -1px 0 #000000, 1px -1px 0 #000000, -1px 1px 0 #000000;');
                }
                ev.stopPropagation();
            };
            ...
            e.fontFamilyFrame.onclick = function(ev) {
                ...
                if (id === "btn-back-options-frame") {
                    ...
                    document.head.appendChild(style);
                    sheet = style.sheet;
                    sheet.addRule('.memeplayer-video::cue', 'font-family: ' + value);
            ...
            e.fontSizeFrame.onclick = function(ev) {
                ...
                if (id === "btn-back-options-frame") {
                    ...
                    document.head.appendChild(style);
                    sheet = style.sheet;
                    sheet.addRule('.memeplayer-video::cue', 'font-size: ' + value);
                }
                ev.stopPropagation();
            };
            ...
            e.fontColorFrame.onclick = function(ev) {
                ...
                if (id === "btn-back-options-frame") {
                    ...
                    sheet = style.sheet;
                    console.log(value);
                    sheet.addRule('.memeplayer-video::cue', 'color: ' + e.setSubColor(value, e.fontOpacity));
            ...
            e.fontOpacityFrame.onclick = function(ev) {
                ...
                if (id === "btn-back-options-frame") {
                    ...
                    document.head.appendChild(style);
                    sheet = style.sheet;
                    sheet.addRule('.memeplayer-video::cue', 'color:' + e.setSubColor(e.currentFontColor, val));
                }
                ev.stopPropagation();
            };
            ...
            e.bgColorFrame.onclick = function(ev) {
                ...
                if (id === "btn-back-options-frame") {
                    ...
                    document.head.appendChild(style);
                    sheet = style.sheet;
                    sheet.addRule('.memeplayer-video::cue', 'background-color:' + e.setSubColor(value, e.bgOpacity));
            ...
            e.bgOpacityFrame.onclick = function(ev) {
                ...
                if (id === "btn-back-options-frame") {
                    ...
                    document.head.appendChild(style);
                    sheet = style.sheet;
                    sheet.addRule('.memeplayer-video::cue', 'background-color:' + e.setSubColor(e.currentBgColor, val));
                }
                ev.stopPropagation();
            };
            ...
            e.subMobileFrame.onclick = function(ev) {
                ...
                if (languageLabel == null) {
                    ...
                    if (lang === 'OFF') {
                        ...
                        btnSub.classList.add('active');
                        var len = e.videoTag.textTracks.length;
                        for (i = 0; i < len; i++) {
                            e.videoTag.textTracks[i].mode = 'hidden';
                        }
                        e.videoTag.textTracks[e.currentLang].mode = 'showing';
        ...
        setDefaultSubLine: function() {
            var track = this.videoTag.textTracks[this.currentLang];
            if (track && track.cues) {
                ...
                if (track.activeCues) {
                    if (track.activeCues[0] !== undefined) {
                        var tt = track.activeCues[0].text;
                        track.activeCues[0].text = "";
                        track.activeCues[0].text = tt;
                        track.activeCues[0].line = 'auto';
                    }
                }
            }
        },
        setSubLine: function() {
            ...
            if (track && track.cues) {
                ...
                if (track.activeCues) {
                    if (track.activeCues[0] !== undefined) {
                        var tt = ee.activeCues[0].text,
                            checkLine = (track.activeCues[0].text.match(/\n/g) || []).length;
                        track.activeCues[0].text = "";
                        track.activeCues[0].text = tt;
                        if (checkLine === 2) {
                            track.activeCues[0].line = 9;
                        } else if (checkLine === 1) {
                            track.activeCues[0].line = 10;
                        } else {
                            track.activeCues[0].line = 11;
                        }
                    }
                }
            }
        ...
        hideSubtitle: function() {
            var self = this,
                tracks = self.videoTag.textTracks,
                len = tracks.length;
            if (tracks && len > 0) {
                for (var i = 0; i < len; i++) {
        ...
        showSubtitle: function() {
            ...
            if (tracks && len > 0) {
                ...
                tracks[self.currentLang].mode = 'showing';
            }
        },
        checkSubOn: function() {
            return this.isSubOn;
    ...
    MemeVideoComp.create("ad", {
        ...
        removeTopAdBackGround: function() {
    ...
    MemeVideoComp.create("adLoading", {
        init: function(p) {
            ...
            e.innerHTML = '<span id="loading-ad-label" style="color: rgb(255, 255, 255); font-weight: bold; position: absolute; text-align: center; top: 45%; width: 100%; font-size: 20px; text-shadow: 3px 1px 3px #000000">Loading Ads...</span>';
    ...
    MemeVideoComp.create("load", {
        ...
        resizePauseAd: function(pauseAdImg, playerWidth, playerHeight, state) {
            ...
            if (imgWidth < playerWidth && imgHeight < playerHeight) { // img size smaller than player size
                pauseAdImg.setAttribute("style", "top: " + (playerHeight - imgHeight) / 2 + "px; left: " + (playerWidth - imgWidth) / 2 + "px; position: absolute;");
            } else { // img size larger than player size
                if (imgRate > 1) {
                    if (imgWidth > playerWidth) {
                        pauseAdImg.setAttribute("style", "width: " + playerWidth + "px; height:" + (playerWidth / imgRate) + "px; top: " + ((playerHeight - (playerWidth / imgRate)) / 2) + "px; position: absolute;");
                    }
                } else {
                    if (imgHeight > playerHeight) {
                        pauseAdImg.setAttribute("style", "width: " + (playerHeight * imgRate) + "px; height:" + (state ? (playerHeight - 30) : playerHeight) + "px; left: " + ((playerWidth - (playerHeight * imgRate)) / 2) + "px; position: absolute;");
                    }
                }
            }
        },
    ...
    MemeVideoComp.create("relatedVideo", {
        ...
        createElementVideoItem: function(player, videoItem, index) {
            ...
            divInfo.addEventListener('click', function(ev) {
                if (ev.path[0].id !== (undefined || "")) {
                    self.updateSource(ev.path[0].id.split('item-index-')[1]);
                }
            });
            divInfo.addEventListener('mouseover', function(ev) {
            });
            aHref.appendChild(divInfo);

            var spanTitle = document.createElement('span');
            spanTitle.className = 'title-relate-video';
            spanTitle.appendChild(document.createTextNode(videoItem.title));
            if (isMobile){
                spanTitle.style.setProperty("font-size", 22 + "px");
                spanTitle.style.setProperty("font-weight", "normal");
            }
            divInfo.appendChild(spanTitle);

            var spanChannel = document.createElement('span');
            spanChannel.className = 'channel-relate-video hidden-mobile';
            if (isMobile){
                spanChannel.style.setProperty("font-size", 22 + "px");
            ...
            spanDuration.className = 'duration-relate-video hidden-mobile';
            spanDuration.style.setProperty("bottom", 5 + "px");
            if (isMobile){
                spanDuration.style.setProperty("font-size", 22 + "px");
            ...
            aHref.addEventListener('click', function(ev) {
                if (ev.path[1].id !== (undefined || "")) {
                    self.updateSource(ev.path[1].id.split('item-index-')[1]);
                }
            });
            return aHref;
        },
        ...
        createElementVideoRelated: function(player, videoList, length) {
            ...
            divRelated.className = 'relate-video ' + className;
            divRelated.style.height = player.size.stageHeight;
            divRelated.style.display = "none";

            var divWrapperRelate = document.createElement('div');
            divWrapperRelate.className = 'wrapper-relate-video';
            ...
            divContentVideo.className = 'content-relate-video';
            divContentVideo.style.backgroundColor = "black";
            divContentVideo.style.setProperty("padding-top", 20 + "px", "important");
            divWrapperRelate.appendChild(divContentVideo);

            for (var i = 0; i < length; i++) {
                divContentVideo.appendChild(this.createElementVideoItem(player, videoList[i], i));
            };
            return divRelated;
        },
        show: function() {
    ...
    var MeCloudVideoPlayer = function(container, data) {
        ...
        this.addEventListener(EVENT.FULLSCREEN, function() {
            ...
            this.components.subtitleBtn.showSubSrtFullscreen(Number.MAX_SAFE_INTEGER);
            this.components.subtitleBtn.setSubFrameFullscreen();
            this.components.subtitleBtn.setSubFrameIndex(Number.MAX_SAFE_INTEGER);
            console.log(this.components.ad);
            this.components.ad.element.style.setProperty("z-index", Number.MAX_SAFE_INTEGER);
            /* var vvv = document.getElementsByClassName('memeplayer-skipbtn')[0];
             if (vvv)
                 vvv.style.setProperty("z-index", Number.MAX_SAFE_INTEGER);*/
        });
        ...
        this.addEventListener(EVENT.EXIT_FULLSCREEN, function() {
            ...
            if (this.isPlaying) {
                ...
                this.components.ad.removeTopAdBackGround();
            }
    ...
    MeCloudVideoPlayer.prototype = {
        ...
        importData: function(data) {
            ...
            if (this.adData && this.adData.pausead && this.adData.pausead.adtag && this.adData.pausead.adtag.length > 0) {
                var ad = this.adData.pausead;
                if (ad.selectRule === SELECT_RULE.RANDOM) {
                    ad.adIndex = Math.floor(Math.random() * ad.adtag.length);
                } else
                    ad.adIndex = ad.adIndex ? (ad.adIndex + 1) % ad.adtag.length : 0;
                var adtag = ad.adtag[ad.adIndex];
                this.components.load.setupPauseAd(adtag.fileLink, adtag.url, adtag.adtagId);
                this.components.load.pauseAd.style.setProperty("display", "none");
            }
            ...
            if (data.subtitle) {
                if (data.subtitle.tracks && data.subtitle.tracks.length > 0) {
                    ...
                    cp.subtitleBtn.changeSubBtnCSS();
                    cp.stage.setTracks(data.subtitle.tracks);
                    cp.subtitleBtn.setLanguageArray(cp.stage.getSubLabel());
                    cp.subtitleBtn.createLanguageFrame(cp.stage.getSubLabel());
                    cp.subtitleBtn.createSubMobileFrame(cp.stage.getSubLabel());
                    cp.subtitleBtn.parseSubtitleSrt(data.subtitle.tracks);



                    document.getElementById('btn-option-language').childNodes[0].innerHTML = "Languages (" + cp.stage.getSubLabel().length + ")";
                    var style = document.createElement("style");
                    document.head.appendChild(style);
                    sheet = style.sheet;
                    sheet.addRule('.memeplayer-video::cue', 'background-color: rgba(255, 255, 255, 0)');
                    sheet.addRule('.memeplayer-video::cue', 'text-shadow: 2px 0 0 #000000, -2px 0 0 #000000, 0 2px 0 #000000, 0 -2px 0 #000000, 1px 1px #000000, -1px -1px 0 #000000, 1px -1px 0 #000000, -1px 1px 0 #000000;');
                    if (isMobile) {
                        sheet.addRule('.memeplayer-video::cue', 'font-size: 30px');
                        cp.subtitleBtn.setFontSizeSrt(30 + "px");
                    } else {
                        sheet.addRule('.memeplayer-video::cue', 'font-size: 22.5px');
                        cp.subtitleBtn.setFontSizeSrt(22.5 + "px");
                    }
                    var isDefaultSub = 0,
                        len = data.subtitle.tracks.length;
                    for (var j = 0; j < len; j++) {
                        if (data.subtitle.tracks[j].isDefault) isDefaultSub = j;
                    }
                    if (data.subtitle.autotrack) {
                        cp.subtitleBtn.showSrtSub();
                        var sub = cp.subtitleBtn;
                        tracks = sub.videoTag.textTracks;
                        checkOnOff = document.getElementById('sc-toggle-1'),
                            btnSub = document.getElementById('btn-subtitles'),
                            k = 1,
                        ...
                        tracks[isDefaultSub].mode = 'showing';
                        sub.setSubOn();
                        sub.setCurrentLang(isDefaultSub);
                        btnSub.classList.add('active');
                        document.getElementById('btn-option-language').childNodes[1].innerHTML = sub.languagesArray[sub.getCurrentLang()] + ' ›';
                        sub.languagesFrame.childNodes[isDefaultSub + 2].innerHTML = '<div class="title-config" style="padding: 5px 10px !important">' + sub.setCheckMark() + "" + sub.languagesFrame.childNodes[isDefaultSub + 2].innerText + '</div>';
                        ...
                        cp.stage.element.textTracks[0].mode = 'hidden';
                    }
                }
            }

            if (data.related && data.related.length > 0) {
                var length = data.related.length > 9 ? 9 : data.related.length;
                length = cp.relatedVideo.getMaxItem(length);
                this.container.appendChild(this.components.relatedVideo.createElementVideoRelated(this, data.related, length));
                this.components.relatedVideo.setVideoItems(data.related);
                this.components.relatedVideo.setRelated(true);
                //var length = videoList.length > 9 ? 9 : videoList.length;
                //length = getMaxItem(length);

                //this.elementRelated = related.createElementVideoRelated(this, videoList, length);
                //this.el_.appendChild(this.elementRelated);
            }
        },
        ...
        updateSize: function(wi, hi) {
            ...
            this.adManager.resize(wi, hi, (d.fullScreen || d.mozFullScreen || d.webkitIsFullScreen || d.msFullscreenElement) ? full : normal);
