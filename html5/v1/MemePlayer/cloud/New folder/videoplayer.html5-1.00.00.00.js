(function (w, d) {
    ...
    var r = d.querySelectorAll('link[href="' + w.$_VConfig.CSS + '"]'), coreFunc = w.MeCloudVideoPlayer.coreFunc;
    if (!r || !r.length) {
    ...
    var CONTROL_BAR_HEIGHT = isMobile ? 50 : 30, inited = {}, players = {};
    ...
    function hash(value, displayKey) {
        var length = 32, result = [], sum = 1024, len = value.length,
                len2 = displayKey.length, rsStr = "";
    ...
    function pause(player, v, t) {
        ...
        try {
            v.pause();
            player.sendEvent("Player Action","Pause");
        } catch (e) {
            console.log("[MemePlayer]", "ERROR PAUSE");
        }
        player.isPlaying = false;
    }
    function pauseNoPing(player) {
        var v = player.components.stage;
        try {
            v.pause();
        } catch (e) {
            console.log("[MemePlayer]", "ERROR PAUSE");
    ...
    function fullscreenchange(event) {
        var state = d.fullScreen || d.mozFullScreen || d.webkitIsFullScreen || d.msFullscreenElement,
                target = state && (d.fullscreenElement || d.mozFullscreenElement || d.webkitFullscreenElement
        ...
        function onAdEvent(adEvent) {
            ...
            switch (adEvent.type) {
                case google.ima.AdEvent.Type.LOADED:
                    ...
                    if (!ad.isLinear()) {
						if(ad.getWidth()<20||ad.getHeight()<20) {
							console.log("[MemePlayer]", "Ad error size");
...
//                        if () {
                ...
                case google.ima.AdEvent.Type.STARTED:
                    ...
                    player.sendEvent("Advertising","play "+adsInfo.position.toUpperCase()+" roll",currentAd.adtagUrl+" ["+player.cfg.vid+"]");
                    displayed++;
    ...
    MemeVideoComp.create("stage", {
        ...
        init: function () {
            ...
            e.onplay = function () {
                ...
                if(!p.sendPlayEvent){
                    p.sendEvent("Player Action","Play");
                    p.sendPlayEvent=true;
                }
                if( !p.firstPlay&&p._ended){
                    p.sendEvent("Player Action","Replay");
                }
            };
            e.onseeked = function () {
                p.ping('sv');
                p.sendEvent("Player Action","Seek");
            };
            e.onpause = function () {
                p.ping('ev');
        ...
        requestFullScreen: function () {
            ...
            this.player.sendEvent("Player Action","Full Screen");
//            this.player.hideAdContainer();
        ...
        exitFullScreen: function () {
            ...
            if (d.webkitExitFullscreen)
                ...
                d.exitFullscreen();
            this.player.sendEvent("Player Action","Normal Screen");
...
//            this.hide();
    ...
    MemeVideoComp.create("volumeBtn", {
        ...
        init: function () {
            ...
            e.onclick = function () {
                ...
                p.sendEvent("Player Action","Mute");
            };
            inner.appendChild(e);
            ...
            e.onclick = function () {
                ...
                p.sendEvent("Player Action","Un Mute");
            };
            inner.appendChild(e);
            p.components.controlbar.appendControl(this);
        }
    ...
    MemeVideoComp.create("qualityBtn", {
        ...
        init: function () {
            ...
            var s = e.style;
            s.setProperty("top", (isMobile ? '11' : '7') + "px", "important");
            s.setProperty("right", (isMobile ? (isIphone ? '54' : '88') : '62') + "px", "important");
            var size = (isMobile ? '24' : '18');
    ...
    MemeVideoComp.create("fullscreenBtn", {
        ...
        init: function () {
            ...
            s.setProperty("right", (isMobile ? '80' : '54') + "px", "important");
...
//            console.log("Ad height " + this.ah + " " + ah);
            var type = ad.getContentType(), ph = this.player.size.height;
...
//            addEvent(e, "click", onclick);
            ...
            if (isMobile) {
                ...
                btn.innerHTML = '<div class="btn-play-mobile" style="margin-left: -32px !important;margin-top: -36px !important;">\
                                    <svg style="margin-left: 15px !important;margin-top: 12px !important;" version="1.1" id="play-moblie" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="40px" height="40px" viewBox="0 0 18 18" enable-background="new 0 0 18 18" xml:space="preserve">\
                                        <path d="M1.002,15.381c0,2.818,3.19,1.062,3.19,1.062s10.62-5.268,10.624-5.312c4.798-1.968,0-4.251,0-4.251s0,0-10.624-5.312c-3.398-1.699-3.19,1.062-3.19,1.062L1.002,15.381L1.002,15.381z"></path>\
                                        </svg>\
                                </div>';
                e.appendChild(btn);
            } else {
                s.setProperty("width", w + "px");//-40
                s.setProperty("height", h + "px", "important");//-40
                s.setProperty("padding", "20px", "important");
                //console.log("player w:"+w);
        ...
        setupPauseAd: function (imgUrl, targetUrl, adtagId) {
            var a = d.createElement("a"), p = this.player, z = this.pauseAd;
            ...
            a.innerHTML = '<img src="' + imgUrl + '" style="max-witdh:100%;max-height:100%"/>';
            z.innerHTML = "";
            z.appendChild(a);
...
//                c.element.className += " hide-controls";
    ...
    MeCloudVideoPlayer.prototype.sendEvent=function(cate,action,label){//alert(cate);
      var ga=w[this.ga_name];
      for (var key in this.accounts) {
        ga(key+'.send', 'event', cate, action, label||this.cfg.title+" ["+this.cfg.vid+"]");
        //alert("ho ho");
      }
  };
    MeCloudVideoPlayer.prototype.initGA=function(config){
        //console.log(config);alert("init");
        this.cfg=config;
        var cfg=config.ga||{id:null,name:"ga"};
        this.ga_name=cfg.name||"ga";
        (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
  })(w,d ,'script','//www.google-analytics.com/analytics.js',this.ga_name);
    var ga=w[this.ga_name];
    this.accounts={"mecloud":"UA-68206175-1"};
    //this.accounts={"mecloud":"UA-51485813-7"};
    if(cfg.id)
    if(cfg.id  instanceof Array){
        for(var i=0;i<cfg.id.length;i++){
            this.accounts["name_"+i]=cfg.id[i];
        }
    }else{
        this.accounts["name_0"]=cfg.id;
    }
    for (var key in this.accounts) {
      ga('create', this.accounts[key], 'auto', {'name': key});
      //ga(key+'.send', 'pageView');
    }
    this.sendEvent("Player Action","Player Loaded");
    this.sendEvent("Player Technology","HTML5");
};
