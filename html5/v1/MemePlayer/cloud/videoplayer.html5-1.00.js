(function(w, d) {
                ...
                case google.ima.AdEvent.Type.STARTED:
                    ...
                    if (ad.isLinear() || !player.isPlaying) {
                        ...
                        if (!ad.b.vpaid) {
                            ...
                            if (contentType && contentType.indexOf("video") === 0) {
                                ...
                                if (!ad.isSkippable() && currentAd.skippable) {
                                    ...
                                    //can't skippable, only show AD_COUNTDOWN
                                    if (currentAd.adType === VAST){
                                        player.showAdControls();
                        ...
                        if (player.components.subtitleBtn.checkSubOn() === true) {
                            player.components.subtitleBtn.hideSrtSub();
                        }
                        var state = d.fullScreen || d.mozFullScreen || d.webkitIsFullScreen || d.msFullscreenElement;
                        if (state)
    ...
    MemeVideoComp.create("stage", {
        ...
        init: function() {
            ...
            e.oncanplay = function() {
                ...
                console.log(/MSIE/i.test(navigator.userAgent));
                console.log(e.src);
            };
            ...
            e.onerror = function(ev) {
                ...
                if (p) {
                    p.ping("sp", ev.target.error.code, {
                    errorCode: ev.target.error.code,
                    errorName: "Media Error Video Html5",
                    platform: "HTML5"
                });
                }
            };
        ...
        play: function() {
            ...
            var vdSize = this.videoSize;
            var videoLength = this.element.duration;
            var vv = this.element;
            var dataAvg = (vdSize / videoLength); // so byte trung binh load trong 10 milisecond.
            var vp = this.player;
            var bytesLoaded = 0;//so byte load dc
            var firstTimeLoad = 0;
            updateProgressBar = function(){
                console.log('update progress bar');
                var buffered = vv.buffered.end(0);
                var percent = 100 * buffered / videoLength;
                bytesLoaded = Math.round(buffered * dataAvg);
                if (bytesLoaded <= 0){
                    firstTimeLoad += 10;
                }
                //console.log(Math.round(buffered * dataAvg) + ' *** ' + percent + ' *** ' + videoLength + ' ' + dataAvg + ' ' + vdSize);
                if (buffered * dataAvg >= vdSize || vp.isEnd){ 
                    clearInterval(watchBuffer);
                }
            }
            
            if (this.player.firstPlayVideo){
                console.log('FIRST PLAY');
                this.player.firstPlayVideo = false;
                var watchBuffer = setInterval(updateProgressBar, 10);
                setTimeout(function(){
                    clearInterval(watchBuffer);
                    console.log(bytesLoaded + ' ' + firstTimeLoad);
                    vp.ping("sp", 0, {
                        latency: firstTimeLoad,
                        byteLoaded: bytesLoaded,
                        platform: "HTML5"
                    });
                }, 10000);
            }
    ...
    MemeVideoComp.create("memeIcon", {
        ...
        changePositionControlItem: function(logoWidth) {
            console.log('change position control item');
            var cp = this.player.components;
            if (isMobile) {
                if (isIphone) {
                    cp.qualityBtn.changeStyleRight(logoWidth + 12);
                    cp.subtitleBtn.changeStyleRight(logoWidth + 43);
                } else {
                    cp.fullscreenBtn.changeStyleRight(logoWidth + 45);
                    cp.qualityBtn.changeStyleRight(logoWidth + 48);
                    cp.subtitleBtn.changeStyleRight(logoWidth + 79);
                }
            } else {
                cp.subtitleBtn.changeStyleRight(logoWidth + 57);
                cp.fullscreenBtn.changeStyleRight(logoWidth + 30);
                cp.qualityBtn.changeStyleRight(logoWidth + 35);
            }
        },
    });
    ...
    MemeVideoComp.create("subtitleBtn", {
        ...
        changeStyleRight: function(value){
            this.element && this.element.style.setProperty("right", value + "px");
            if (isIE) {
                this.element && this.element.style.setProperty("right", value + "px", "important");
            }
        }
    ...
    MemeVideoComp.create("thumb", {
        init: function() {
            ...
            s.setProperty("-moz-transform", "translateX(-50%)");
            s.setProperty("-ms-transform", "translateX(-50%)");
    ...
    MeCloudVideoPlayer.prototype = {
        ...
        playInfo: null,
        session: '',
        firstPlayVideo: true, // su dung cho viec handle player.ping("sp", 0, {});
        ...
        importData: function(data) {
            ...
            if (data.video[0].size){
                cp.stage.setVideoSize(data.video[0].size);
            }
